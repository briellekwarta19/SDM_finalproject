...
#at next 10 downstream locations
...
#initially remove at 5 downstream locations
site.rem <- array(NA, c(n.rem,n.year,n.sims))
site.rem[,1,] <- seq(1,5)
...
#### Path Name ####
path <- here::here("Final_Project", "results", "remdown5_obsdown10")
res <- c('Final_Project/results/remdown5_obsdown10') #subset of path for plot save
...
for(year in 1:years){
  ...
  if(year < years){
    ...
    #Monitoring decision: remove at next 10 most downstream from most downstream removal
...
file_name = paste(path, 'rdown5_odown10_gamma0est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown10_gamma1est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown10_epsest.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown10_pest.csv',sep = '/')
...
file_name = paste(path, 'rdown5_odown10_segrem.csv',sep = '/')
