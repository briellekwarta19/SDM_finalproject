...
#### Path Name ####
path <- here::here("Final_Project", "results", "remdown5_noobs")
res <- c('Final_Project/results/remdown5_noobs') #subset of path for plot save
  ...
  for(s in 1:n.sims){
    ...
    assign(outs[s],
           jagsUI::jags(data = my.data[[s]],inits = initial.values,
                        parameters.to.save = parameters.to.save, model.file = "Flower_mod_noobs.txt",
...
file_name = paste(path, 'rdown5_noobs_gamma0est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_noobs_gamma1est.csv',sep = '/')
...
file_name = paste(path, 'rdown5_noobs_epsest.csv',sep = '/')
